...
public class StreamAssignment {
    ...
    public void task5(){
        ...
        Set<LocalDate> dates = people.stream().map(Person::getDateOfBirth).collect(TreeSet::new, TreeSet::add, TreeSet::addAll);
    ...
    public void task9(){
        ...
        List<PersonDto> dtoList = people.stream().filter(person -> person.getDateOfBirth().isBefore(date)).map(person -> new PersonDto(person.getPersonId(), person.getFirstName() + " " + person.getLastName())).collect(Collectors.toList());
    ...
    public void task10(){
        ...
        Optional<String> optional = people.stream().filter(person -> person.getPersonId() == personId).findFirst().map(person -> person.getDateOfBirth().format(DateTimeFormatter.ofPattern("EEEE dd MMMM yyy")).toUpperCase());
    ...
    public void task11(){
        ...
        double averageAge = people.stream().mapToInt(personToAge).average().orElseThrow(IllegalStateException::new);
    ...
    public void task12(){
        ...
        String[] result = people.stream().map(Person::getFirstName).filter(person -> IntStream.range(0, person.toLowerCase().length() / 2).
                noneMatch(index -> person.toLowerCase().charAt(index) != person.toLowerCase().charAt(person.toLowerCase().length() - index - 1))).
    ...
    public void task14(){
        ...
        LocalDate[] _2020_dates = Stream.iterate(startDate, localDate -> localDate.plusDays(1)).
                limit(ChronoUnit.DAYS.between(startDate, endDate)).toArray(LocalDate[]::new);
